...
|------|--------|------|--------|---------|---------|--------|
| **¥5,000** | **RK1828 M.2 算力卡 + RK3588 主机** | 瑞芯微 RK1828（2025 量产） | 20 TOPS NPU + 5GB 堆叠 DRAM | **Qwen2.5-7B-Instruct（INT4）** | **50-80 tok/s** | 买速度——NPU 硬加速，7B 模型聊得动 |
| **¥10,000** | **Milk-V Pioneer** | 算能 SG2042（64 核 C920） | 64 核 CPU + 128GB ECC 内存 | **Qwen2.5-7B-Instruct（日常）/ Qwen2.5-14B-Instruct（复杂任务）** | **~6.6 tok/s / ~2.3 tok/s** | 买容量——唯一能跑 14B+ 模型的 RISC-V 机器 |

> **为什么 ¥5,000 比 ¥10,000 在 7B 上快 8 倍？** RK1828 是专用 NPU（20 TOPS 硬加速），Pioneer 是纯 CPU（64 核软跑）。NPU 做矩阵乘法天生比 CPU 快一个数量级。但 Pioneer 有 128GB 内存——RK1828 最多装 7B，Pioneer 能装 14B、70B。2026 年 RISC-V 市场没有一个既快又能跑大模型的单品。

### 为什么推荐 Qwen2.5 而不是 DeepSeek

RK1828 上实测数据：

| 模型 | 实测速度 | TTFT | 来源 |
|------|---------|------|------|
| **Qwen2.5-7B-Instruct** | **50-80 tok/s** | ~160ms | 瑞芯微官方 + iotdt.com 实测 |
| DeepSeek-7B | 60-80 tok/s | — | iotdt.com |
| DeepSeek-R1-Distill-Qwen-7B | 56 tok/s | 159ms | 瑞芯微官方实测 |

三个模型速度在同一区间。选择 Qwen2.5 的理由：

1. **同一速度，没有差距。** 50-80 vs 56-80 tok/s 都是能用级别，体感无差异。
2. **Qwen2.5 在 RK1828 上优先级最高。** 瑞芯微将 Qwen 系列列为"深度优化，实测可跑"第一梯队，DeepSeek 也在第一梯队但排第二。
3. **Qwen 和玄铁同属阿里生态。** 你的开发板大概率用玄铁 IP，未来玄铁 C950 原生适配 Qwen3。统一生态降低适配成本。
4. **DeepSeek-R1 的推理链是双刃剑。** 数学/代码更强，但每次回复先生成思考链再输出答案，总 token 消耗更大，实际等更久。56 tok/s 是生成速度，但你要等它"想"完才看到答案。
5. **中文场景 Qwen2.5 仍是 SOTA。** DeepSeek 强在英文推理 benchmark，日常中文对话 Qwen 有优势。

**DeepSeek 什么时候用：** 你需要模型做复杂推理（数学证明、代码审计、逻辑推理）时切换到 DeepSeek-R1。RK1828 两个都支持，都测过，随时换。
...
**推荐模型：Qwen2.5-7B-Instruct（INT4，约 3.8GB）**

选择依据：
1. 速度 50-80 tok/s，和其他 7B 在同一区间，无差异
2. 瑞芯微将 Qwen 系列列为第一优先级深度优化模型
3. Qwen + 玄铁同属阿里生态——将来 C950 平台可无缝迁移
4. 中文 SOTA，中文场景能力最强

**什么时候切 DeepSeek-R1（56 tok/s）：** 需要复杂推理（数学、代码审计、逻辑分析）时。推理链让模型在输出结论前"思考"，生成的答案质量更高，但等得更久——56 tok/s 是生成速度，加上思考链，实际拿到答案的时间比 Qwen2.5 长。
...
**推荐策略：双模型——日常用 7B，复杂任务用 14B**

Pioneer 有 128GB 内存，唯一的优势就是能装大模型。分两个场景：

| 场景 | 模型 | 速度 | 用途 |
|------|------|------|------|
| **日常** | **Qwen2.5-7B-Instruct（INT4）** | ~6.6 tok/s | 交互式对话、代码补全、文档问答 |
| **复杂任务** | **Qwen2.5-14B-Instruct（INT4）** | ~2.3 tok/s | 代码审查、长文摘要、多步推理、prompt 工程验证 |

7B 模型 6.6 tok/s 是 Pioneer 上最快的可用速度——虽然不如 RK1828 的 50+ tok/s，但能交互。14B 模型 2.3 tok/s 是批处理速度：发任务，去喝杯咖啡，回来拿结果。

选择 Qwen2.5-14B 而不是 DeepSeek-R1-14B 的原因：SG2042 只有 DeepSeek-R1-14B 的实测数据（2.29 tok/s），但同参数量的 Qwen2.5-14B-Instruct 同等量化下速度应该持平或略快（无推理链开销）。中文能力更强，与 RK1828 端统一模型系列降低维护成本。

**Pioneer 的真正价值：** 128GB ECC 内存能塞 70B 模型做评估。64 核并行编译内核、跑 CI、搭数据库。它是 RISC-V 开发工作站，AI 推理只是其中一部分能力。
